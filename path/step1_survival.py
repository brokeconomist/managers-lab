...
def run_step():
    ...
    # 1. DYNAMIC SYNC FROM SESSION STATE
    ...
    # FETCH FIXED COSTS FROM STAGE 0 (Defaults to 50000 if not found)
    shared_fixed_costs = st.session_state.get('fixed_costs', 50000.0)
    
    # 2. DATA INTEGRITY CHECK
    if price <= 0 or current_volume <= 0:
        st.warning("⚠️ Please return to Stage 0 and define Price and Volume.")
        if st.button("⬅️ Back to Stage 0"):
            st.session_state.flow_step = 0
            st.rerun()
        return

    # 3. FIXED COSTS INPUT (Linked to shared state)
    st.subheader("Annual Fixed Costs")
    col1, col2 = st.columns(2)
    with col1:
        # The 'value' is now driven by the session_state directly
        fixed_costs = st.number_input("Total Annual Fixed Costs (€)", 
                                      min_value=0.0, 
                                      value=float(shared_fixed_costs),
                                      step=1000.0)
        # Immediate update of the state to keep it in sync
    ...
    fig.update_layout(title="Break-Even Chart", xaxis_title="Annual Units", yaxis_title="Euros (€)", template="plotly_dark")
